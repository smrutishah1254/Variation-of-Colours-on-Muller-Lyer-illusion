...
expName = 'Muller_Lyer_1'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\ADMIN\\Desktop\\SOLO PROJECT\\Muller_Lyer_1_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text = visual.TextStim(win=win, name='text',
        text="In this experiment, you will see two lines on the screen. Your task is to judge the relative length of the lines and respond as follows:\n\nPress the 'Z' key if you think the upper/ left line is longer.\nPress the 'M' key if you think the lower/right line is longer.\nPress the 'spacebar' if you think both lines are of equal length. \n\nPlease respond as quickly and accurately as possible. Take your time to understand the instructions and key 'spacebar' to begin with the experiment :)",
        font='Arial',
    ...
    # --- Initialize components for Routine "Trial1_Same_colors" ---
    ...
    # --- Initialize components for Routine "Trial2_Diff_colors" ---
    ...
    trials = data.TrialHandler(nReps=5.0, method='random', 
        ...
        trialList=data.importConditions('Images_file_same_1.csv'),
    ...
    for thisTrial in trials:
        ...
        # --- Prepare to start Routine "Trial1_Same_colors" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('Trial1_Same_colors.started', globalClock.getTime(format='float'))
        image.setSize((0.7, 0.6))
        ...
        _key_resp_allKeys = []
        # keep track of which components have finished
        Trial1_Same_colorsComponents = [polygon, image, key_resp]
        for thisComponent in Trial1_Same_colorsComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "Trial1_Same_colors" ---
        routineForceEnded = not continueRoutine
        while continueRoutine:
            # get current time
            t = routineTimer.getTime()
            ...
            if key_resp.status == STARTED and not waitOnFlip:
                theseKeys = key_resp.getKeys(keyList=['z','m','space'], ignoreKeys=["escape"], waitRelease=False)
        ...
        # --- Ending Routine "Trial1_Same_colors" ---
        for thisComponent in Trial1_Same_colorsComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('Trial1_Same_colors.stopped', globalClock.getTime(format='float'))
        # check responses
        ...
        # the Routine "Trial1_Same_colors" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
    ...
    trials_2 = data.TrialHandler(nReps=5.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('Images_file_diff_1.csv'),
    ...
    for thisTrial_2 in trials_2:
        ...
        # --- Prepare to start Routine "Trial2_Diff_colors" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('Trial2_Diff_colors.started', globalClock.getTime(format='float'))
        image2.setSize((0.7, 0.6))
        image2.setImage(Image_file)
        # create starting attributes for key_resp2
        key_resp2.keys = []
        key_resp2.rt = []
        _key_resp2_allKeys = []
        # keep track of which components have finished
        Trial2_Diff_colorsComponents = [polygon2, image2, key_resp2]
        for thisComponent in Trial2_Diff_colorsComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "Trial2_Diff_colors" ---
        routineForceEnded = not continueRoutine
        while continueRoutine:
            # get current time
            t = routineTimer.getTime()
            ...
            # if polygon2 is starting this frame...
            if polygon2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                ...
                # update status
                polygon2.status = STARTED
                polygon2.setAutoDraw(True)
            
            # if polygon2 is active this frame...
            if polygon2.status == STARTED:
                # update params
                pass
            ...
            if key_resp2.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if key_resp2.status == STARTED and not waitOnFlip:
                theseKeys = key_resp2.getKeys(keyList=['z','m','space'], ignoreKeys=["escape"], waitRelease=False)
                ...
                if len(_key_resp2_allKeys):
                    ...
                    # was this correct?
                    if (key_resp2.keys == str(Correct_ans)) or (key_resp2.keys == Correct_ans):
                        key_resp2.corr = 1
                    else:
                        key_resp2.corr = 0
                    # a response ends the routine
        ...
        # --- Ending Routine "Trial2_Diff_colors" ---
        for thisComponent in Trial2_Diff_colorsComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('Trial2_Diff_colors.stopped', globalClock.getTime(format='float'))
        # check responses
        ...
        # the Routine "Trial2_Diff_colors" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
